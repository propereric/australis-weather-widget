...
function TestWidget () {
          ...
          var x = pageWorkers.Page({
              ...
              onMessage: function(wObject) {
                ...
                if(document) {
                  ...
                  //refresh the display only if there is a document
	              outputCityElements();
	            }
              }
              });
  ...
  function onSetIcon() {
    if(cityList.length > 0) {
      ...
      document.getElementById("msu-weather-icon").setAttribute("style","border: solid 2px gray;");
      document.getElementById("msu-weather-icon").setAttribute("width", "70px");
      document.getElementById("msu-weather-icon").setAttribute("height", "40px");
    }
  }
  ...
  function outputElement(i, inEdit) {
    ...
    if(cityList[i].rank == 0){ tbl.setAttribute("style","text-align: center; border-radius: 5px;border: 1px solid white;background-color:#C6E2aF; margin-top: 5px;margin-bottom: 10px"); }
    else { tbl.setAttribute("style","text-align: center; margin-bottom: 10px; border: 1px solid gray; background-color: white;"); }

    let tr1 = document.createElement("vbox");
      let td1_1 = document.createElement("image");
        td1_1.setAttribute("src", cityList[i].image);
        td1_1.width = "80px";
        td1_1.height = "70px";
      let td1_2 = document.createElement("label");
        td1_2.innerHTML = cityList[i].temp + " Fahrenheit";

    let tr2 = document.createElement("vbox");
      let td2_1 = document.createElement("label");
        td2_1.className = "text-link";
        td2_1.href = 'http://weather.yahoo.com/_/_/_-'+cityList[i].woeid+'/';
        td2_1.innerHTML = cityList[i].name;
  ...
  ******************************/
  ...
  function injectUI () {
    ...
    let customizeButton = document.createElement("toolbarbutton");
      customizeButton.setAttribute('style','margin-bottom: 10px; box-shadow: 0px 2px 12px #888888; background-color: gray; color: white');
      customizeButton.setAttribute("id", "msu-customize-button");
      customizeButton.innerHTML = "Customize";
      customizeButton.addEventListener("command", customizeClicked);
    ...
    let addCityButton = document.createElement("toolbarbutton");
      ...
      addCityButton.addEventListener("command", addCityClicked);
  ...
  return {
    ...
    widgetCreated: function(node) {
      ...
      let img = doc.createElement("image");
        ...
        img.setAttribute("src", require("sdk/self").data.url("icon.png"));
      ...
      //let img2 = doc.createElement("box");
//        img2.setAttribute("class", "toolbarbutton-icon");
//        img2.id = "msu-weather-icon2";
//        img2.innerHTML = "15";
//        box.appendChild(img);
