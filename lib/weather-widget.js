...
function WeatherWidget () {
  ...
  let iconImage = data.url("icon.png");
      ...
      onComplete: function (response) 
      {
        ...
        if(response.status == 200) {
          var x = pageWorkers.Page({
              ...
              contentScriptFile: [data.url("parse_xml.js"),
              ...
              onMessage: function(wObject) {
                ...
                if(document) {
	              ...
	              var y = pageWorkers.Page({
                    contentURL: data.url("page_worker.html"),
                    contentScriptFile: data.url("image_create.js"),
  ...
  function onSetIcon() {
    if(cityList.length > 0) {
      ...
      document.getElementById("msu-weather-icon").setAttribute("class","msu-large-icon");
      iconImage = cityList[0].image;
      //replace top two with callback to all windows setting src and class
    }
    else {
      document.getElementById("msu-weather-icon").setAttribute("src", data.url("icon.png"));
      document.getElementById("msu-weather-icon").setAttribute("class","msu-small-icon");
      cityList[0].image = data.url("icon.png");
      //replace top two with callback to all windows setting src and class
    }
  }
  ...
  function outputElement(i, inEdit) {
    ...
    if(cityList[i].rank == 0){ tbl.setAttribute("class","msu-primary-element"); } 
    else { tbl.setAttribute("class","msu-secondary-element"); } 
  ...
  ******************************/
  ...
  function injectUI () {
    ...
    let customizeButton = document.createElement("toolbarbutton");
      ...
      //customizeButton.setAttribute("disabled", "true");
  ...
  return {
    ...
    viewShowing: function (doc, theView) {
      ...
      let xmlPI = document.createProcessingInstruction('xml-stylesheet', 'href="'+css+'" type="text/css"'); // Create an XML pi
